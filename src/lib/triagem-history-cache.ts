...
export function getTriagemHistoryCached(leadId: string): TriagemEvent[] | undefined {
  return historyCache.get(leadId);
}

export function setTriagemHistoryCached(leadId: string, events: TriagemEvent[]) {
  historyCache.set(leadId, events);
}

export function prependTriagemHistoryCached(leadId: string, event: TriagemEvent) {
