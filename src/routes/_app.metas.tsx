    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {metas.map((meta) => {
        ...
        return (
          ...
          >
            <div className={`absolute inset-x-0 top-0 h-1 ${visual.progress}`} />
            <CardHeader className="pb-2 pt-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <Badge className={meta.origem === "gerente" ? "border-amber-500/25 bg-amber-500/15 text-amber-700 dark:text-amber-300" : "border-sky-500/25 bg-sky-500/15 text-sky-700 dark:text-sky-300"} variant="outline">
            ...
            <CardContent className="space-y-4 pb-4">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Realizado</p>
                  <p className="mt-1 text-2xl font-bold tracking-tight">{formatValor(meta.atual, meta.tipo)}</p>
                </div>
                <div className="rounded-lg bg-muted/70 px-3 py-2 text-right">
                  ...
                  <p className="text-sm font-semibold">{formatValor(meta.valor, meta.tipo)}</p>
                </div>
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <span className={concluida ? "flex items-center gap-1 font-medium text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-muted">
                  <div className={`h-full rounded-full transition-all ${concluida ? "bg-emerald-500" : visual.progress}`} style={{ width: `${meta.percentual}%` }} />
                </div>
              </div>
              <div className="flex items-center justify-between gap-2 border-t pt-3">
                ...
                {editavel && (
                  <div className="flex shrink-0 gap-1 opacity-70 transition-opacity group-hover:opacity-100">
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => onEdit(meta)} aria-label="Editar meta"><Pencil className="h-3.5 w-3.5" /></Button>
                    <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => onRemove(meta)} aria-label="Excluir meta"><Trash2 className="h-3.5 w-3.5" /></Button>
                  </div>
                )}
              </div>
            </CardContent>
